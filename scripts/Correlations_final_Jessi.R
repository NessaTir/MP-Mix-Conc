...
#compare our regular regression fit to the GAM fit
...
#-> don't know how they can be different if the values are exact the same
...
mod_lm = gam(value ~ conc, data = calcification_Pve)
summary(mod_lm)
...
mod_gam1 = gam(value~ s(conc, bs = "cr"), data = calcification_Pve)
summary(mod_gam1)
...
mod_lm = gam(value ~ conc, data = calcification_Spi)
summary(mod_lm)
...
mod_gam1 = gam(value~ s(conc, bs = "cr"), data = calcification_Spi)
summary(mod_gam1)
